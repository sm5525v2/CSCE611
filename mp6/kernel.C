...
void fun2() {
    ...
    for(int j = 0;; j++) {
       ...
       /* -- Read */
       Console::puts("Thread 2 Reading a block from disk...\n");
       SYSTEM_DISK->read(read_block, buf);
	Console::puts("===================== Thread 2 Read finished===============\n");
       /* -- Display */
       for (int i = 0; i < DISK_BLOCK_SIZE; i++) {
           Console::putch(buf[i]);
       }

       Console::puts("Thread 2 Writing a block to disk...\n");
...
void fun3() {
    ...
    for(int j = 0;; j++) {
       ...
       /* -- Read */
       Console::puts("Thread 3 Reading a block from disk...\n");
       SYSTEM_DISK->read(read_block, buf);
        Console::puts("=====================Thread3 Read finished===============\n");
       /* -- Display */
       for (int i = 0; i < DISK_BLOCK_SIZE; i++) {
           Console::putch(buf[i]);
       }

       Console::puts("Thread 3 Writing a block to disk...\n");
       SYSTEM_DISK->write(write_block, buf);
...
int main() {
    ...
    SYSTEM_SCHEDULER->setDisk(SYSTEM_DISK);
